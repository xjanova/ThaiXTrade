...
│  │    (Chain ID: 7000)  │◄─►│  • Swap (Multi-chain)    │  │
...
| **Chain ID** | 7000 (mainnet), 7001 (testnet) |
| **Decimals** | 18 |
...
| **Staking Rewards** | 20% | 1,400,000,000 | รางวัลสำหรับ stakers |
...
- ถอนก่อนกำหนด: ปรับ 10% (แจกให้ stakers ที่เหลือ)
- Stake ขั้นต่ำ: 1 TPIX

### 4.2 Investment/ROI Staking
...
• RPC endpoints            • TPIX/BNB, TPIX/USDT     • Staking on both
                           • Trade on TPIX TRADE
```
...
| 1.3 | Deploy TPIX Chain Mainnet (Chain ID: 7000) | TODO |
...
| 1.8 | ทดสอบ testnet (Chain ID: 7001) ก่อน mainnet | TODO |
...
| ขั้นตอน | รายละเอียด |
|---|---|
| 3.1 | เพิ่ม TPIX Chain (7000) ใน `config/chains.php` |
...
-- Staking positions (TPIX TRADE side)
...
# Staking
GET    /api/v1/staking/pools           # รายการ staking pools
POST   /api/v1/staking/stake           # stake TPIX
POST   /api/v1/staking/unstake         # ถอน stake
GET    /api/v1/staking/positions       # positions ของ wallet
...
- [ ] ตั้งค่า TPIX Chain testnet (Chain ID: 7001)
...
- [ ] Deploy TPIX Chain mainnet (Chain ID: 7000)
- [ ] Bridge relay service (production)
- [ ] Staking UI ใน TPIX TRADE
...
| Liquidity fragmentation | กลาง | Incentivize liquidity ด้วย staking rewards |
...
│   │   └── StakingController.php         # [ใหม่] Staking API
│   ├── Services/
│   │   ├── BridgeService.php             # [ใหม่] Bridge logic
│   │   ├── TPIXPriceService.php          # [ใหม่] ราคา TPIX
│   │   └── StakingService.php            # [ใหม่] Staking logic
│   └── Models/
│       ├── BridgeTransaction.php         # [ใหม่]
│       └── StakingPosition.php           # [ใหม่]
├── config/
...
│   └── xxxx_create_staking_positions.php
├── resources/js/
│   ├── Pages/
│   │   ├── Bridge.vue                    # [ใหม่] Bridge page
│   │   └── Staking.vue                   # [ใหม่] Staking page
│   ├── Components/
│   │   ├── Bridge/
│   │   │   ├── BridgeForm.vue            # [ใหม่]
...
│   │   └── Staking/
│   │       ├── StakingPools.vue          # [ใหม่]
│   │       ├── StakingForm.vue           # [ใหม่]
│   │       └── StakingPositions.vue      # [ใหม่]
│   └── Composables/
│       ├── useTPIXPrice.js               # [ใหม่] real-time TPIX price
│       └── useBridge.js                  # [ใหม่] bridge composable
└── routes/
    └── api.php                           # [แก้ไข] เพิ่ม bridge + staking routes
```
