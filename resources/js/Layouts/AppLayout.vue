...
/**
 * ลิงก์ใน footer — แหล่งเดียวของ "ทุกหน้าที่ผู้ใช้เข้าถึงได้"
 * เพิ่มหน้าใหม่แล้วเติมที่นี่ที่เดียว · ป้ายชื่อเป็นคีย์คำแปลเสมอ
 * มีเทสต์ FooterLinksTest ยิงทุก href ว่ายังตอบ 200 อยู่จริง
 */
const footerColumns = [
    {
        titleKey: 'footer.trading',
        links: [
            { href: '/trade', labelKey: 'footer.spotTrading' },
            { href: '/swap', labelKey: 'nav.swap' },
            { href: '/markets/spot', labelKey: 'nav.markets' },
            { href: '/markets', labelKey: 'footer.allMarkets' },
            { href: '/portfolio', labelKey: 'nav.portfolio' },
            { href: '/ai-trade', labelKey: 'footer.aiTrade' },
        ],
    },
    {
        titleKey: 'footer.ecosystem',
        links: [
            { href: '/token-sale', labelKey: 'nav.tokenSale' },
            { href: '/launch', labelKey: 'footer.fairLaunch' },
            { href: '/token-factory', labelKey: 'nav.tokenFactory' },
            { href: '/carbon-credits', labelKey: 'nav.carbonCredit' },
            { href: '/food-passport', labelKey: 'footer.foodPassport' },
            { href: '/masternode', labelKey: 'nav.masternode' },
            { href: '/validators', labelKey: 'footer.validators' },
        ],
    },
    {
        ...
        links: [
            ...
            // เดิมลิงก์นี้ชี้ /whitepaper เหมือนบรรทัดบน — กดแล้วไม่ได้ไฟล์ที่โฆษณาไว้
            { href: '/whitepaper/download', labelKey: 'footer.downloadWhitepaper' },
            { href: '/carbon-credits/whitepaper', labelKey: 'footer.carbonDocs' },
            { href: '/explorer', labelKey: 'nav.explorer' },
            { href: '/bridge', labelKey: 'nav.bridge' },
    ...
    },
    {
        ...
        links: [
            { href: '/download', labelKey: 'footer.downloadApp' },
            { href: '/settings', labelKey: 'nav.settings' },
            { href: 'https://xmanstudio.com', labelKey: 'footer.xmanStudio', external: true },
        ],
    },
];
...
<template>
    <div class="flex-1 flex flex-col bg-dark-950">
        ...
        <!-- Footer — ทางเข้าทุกหน้าของระบบ
             ลิงก์มาจากตาราง footerColumns ใน <script setup> เพื่อให้เพิ่ม/แก้หน้าได้ที่เดียว
             และป้ายชื่อทุกอันผูกกับคีย์คำแปล (สลับภาษาแล้วเปลี่ยนตาม) -->
        <footer class="relative border-t border-white/5 mt-8 overflow-hidden">
            <PageArt art="footer-bg" :opacity="26" fade="bottom" position="center" />
            ...
            <div class="relative max-w-[1920px] mx-auto px-6 py-10">
                ...
                <div class="grid grid-cols-2 sm:grid-cols-4 gap-8 mb-8">
                    <div v-for="column in footerColumns" :key="column.titleKey">
                        <h4 class="text-sm font-semibold text-white mb-3">{{ t(column.titleKey) }}</h4>
                        <ul class="space-y-2">
                ...
                <!-- โซเชียล -->
                <div class="mb-6">
                        <div v-if="social.twitter || social.telegram || social.discord || social.github" class="flex items-center gap-3">
                ...
                <div class="border-t border-white/5 pt-6 flex flex-col sm:flex-row items-center justify-between gap-3">
                    <div class="flex items-center gap-3 flex-wrap justify-center">
                        <span class="text-xs text-dark-400">
                            &copy; {{ new Date().getFullYear() }} Xman Studio. {{ t('footer.rights') }}
                        </span>
                        <span class="text-xs text-dark-600">·</span>
                        <span class="text-xs text-dark-500 font-mono">{{ t('footer.chainId') }}</span>
                    </div>
                    <span class="text-xs text-dark-500 font-mono">TPIX TRADE v{{ versionData.version }}</span>
                </div>
            </div>
        </footer>
